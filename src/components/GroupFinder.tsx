...
export function GroupFinder() {
  ...
  return <><section className="finder-section" id="troba-el-teu-grup" aria-labelledby="finder-title"><div className="finder-copy"><h2 id="finder-title">Troba el teu grup de WhatsApp</h2><p>Selecciona d’on vens i el teu institut per trobar el grup del vostre viatge de final de curs a Menorca.</p></div><div className="finder">
    ...
    <div className="finder__field"><SearchableSelect key={`location-${comarcaId}`} id="location" label="Població" value={locationId} options={locations} loading={loading.locations} disabled={!comarcaId || loading.locations} placeholder="Cerca una població" onChange={(value) => void changeLocation(value)} /></div>
    <div className="finder__field"><SearchableSelect key={`institute-${locationId}`} id="institute" label="Institut" value={instituteId} options={institutes} loading={loading.institutes} disabled={!locationId || loading.institutes} placeholder="Cerca un institut" onChange={(value) => void changeInstitute(value)} /></div>
